...
echo "📋 1. 检查Fabric网络状态..."
echo "   🔍 检查容器状态:"
docker ps --format "table {{.Names}}\t{{.Status}}" | grep -E "(peer|orderer)" || echo "   无Fabric容器运行"
...
echo "📋 2. 检查ESG链码状态..."
echo "   🔍 检查ESG链码容器:"
docker ps | grep "dev-peer0.org1.example.com-esg" && echo "   ✅ ESG链码容器运行中" || echo "   ❌ ESG链码容器未运行"
...
ESG_RESPONSE=$(curl -s -X POST http://localhost:8199/api/esg/upload \
  ...
  -d '{"fileName":"test-esg-report.pdf","fileContent":"This is a test ESG report content for blockchain verification","fileType":"pdf","description":"Test ESG report"}' 2>/dev/null)
...
echo "📋 5. 检查服务日志..."
echo "   📄 最新日志:"
tail -20 /tmp/esg_test.log 2>/dev/null | grep -E "(ERROR|WARN|SUCCESS|INFO)" || echo "   无相关日志"
